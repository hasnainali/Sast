...
if (isset($_GET["callbk_id"]))				{$callbk_id=$_GET["callbk_id"];}
	elseif (isset($_POST["callbk_id"]))	{$callbk_id=$_POST["callbk_id"];}
...
if ($function == 'external_pause')
	{
		...
		$log_file_name = "drop_callback_logs/LOG-tekdial_external_pause".$current_date.".log";
		writeErroIntoLogFile(PHP_EOL.PHP_EOL.'*******************TekDial external pause initiated*******************',$log_file_name);
	if ( (strlen($value)<1) or ( (strlen($agent_user)<1) and (strlen($alt_user)<1) ) or (!preg_match("/PAUSE|RESUME/",$value)) )
		...
		if ($row[0] > 0)
			...
			$stmt="UPDATE vicidial_live_agents set external_pause='$value!$epoch',status='PAUSED' where user='$agent_user';";
				if ($format=='debug') {echo "\n<!-- $stmt -->";}
			$rslt=mysql_to_mysqli($stmt, $link);
			$result = _QXZ("SUCCESS");
			$result_reason = _QXZ("external_pause function set");
			echo "$result: $result_reason - $value|$epoch|$agent_user\n";
			writeErroIntoLogFile(PHP_EOL.PHP_EOL.'$result: $result_reason - $value|$epoch|$agent_user',$log_file_name);
			$data = "$epoch";
			api_log($link,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data);
			}
		else
			...
			if ( ($agent_ready > 0) or (strlen($dial_ingroup)>0) )
				...
				if ($row[0] > 0)
					...
					if ($api_manual_dial=='STANDARD')
						...
						if ($row[0] < 1)
							{
							$stmt="INSERT INTO vicidial_manual_dial_queue set user='$agent_user',phone_number='$value',entry_time=NOW(),status='READY',external_dial='$value!$phone_code!$search!$preview!$focus!$vendor_id!$epoch!$dial_prefix!$group_alias!$caller_id_number!$vtiger_callback_id!$lead_id!$alt_dial!$dial_ingroup!$callbk_id';";
							$success=1;
							}
						else
							{
