<?php
require_once("/srv/www/htdocs/tekdial/dbconnect_mysqli.php");
require_once("/srv/www/htdocs/tekdial/functions.php");
$uploadDataLog = '/var/log/tekdial/CreateNewList/'.date('Y-m-d').'-ListUpload.log';

writeErroIntoLogFile('*************createNewList API Initiated**************',$uploadDataLog);
$todaydate = date("Y-m-d")." 00:00:00";
$get_all_campaign = "SELECT `id` FROM `dms_list_id` WHERE `date`='$todaydate'";

writeErroIntoLogFile('Get All dms_list_id Query => '.$get_all_campaign,$uploadDataLog);
$result = mysql_to_mysqli($get_all_campaign, $link);
$camp_rows = mysqli_fetch_assoc($result);
$tekdial_list_id =$camp_rows['id'];
$cam_num_rows = mysqli_num_rows($result);
writeErroIntoLogFile('Get All dms_list_id Query Num Row => '.$cam_num_rows,$uploadDataLog);


$get_all_agents ="select distinct(agent_id) FROM dms_retailers_data WHERE tech_dial_list_id ='".$tekdial_list_id."' GROUP BY agent_id";
$result1 = mysql_to_mysqli($get_all_agents, $link);
$get_all_agent_Arr =array();
while ($agent_rows = mysqli_fetch_assoc($result1)) {
       $get_all_agent_Arr[]=$agent_rows['agent_id'];
	}

$failed_count = 0;
if(!empty($get_all_agent_Arr)){
	// while ($camp_rows = mysqli_fetch_assoc($result)) {
	foreach($get_all_agent_Arr as $agent){
	...
	//die();
		 ...
		 $list_id = str_replace("-", "", $list_id);
	   	 $list_id = str_replace(" ", "", $list_id);
// $list_id;
	   	$list_name = "Dms Another Calling List";
		writeErroIntoLogFile('***LIST ID => '.$list_id,$uploadDataLog);
		// $campaign_id = "DIALER";
		// $campaign_id = $camp_rows['campaign_id'];
		writeErroIntoLogFile('Selected Agent Start**** => '.$agent,$uploadDataLog);
		...
		$date = date('Y-m-d');
		$final_array = array();
		$stmt_count_9_6 = "SELECT * FROM dms_retailers_data WHERE Optional1 = '$date' AND  from_time = '09:00' AND to_time = '18:00' AND agent_id ='$agent' AND tech_dial_list_id ='$tech_dial_list_id'";
		...
		$stmt_count_9_11 = "SELECT * FROM dms_retailers_data WHERE Optional1 = '$date' AND  from_time = '09:00' AND to_time = '11:00' AND agent_id ='$agent' AND tech_dial_list_id ='$tech_dial_list_id'";
		...
		$stmt_count_11_13 = "SELECT * FROM dms_retailers_data WHERE Optional1 = '$date' AND  from_time = '11:00' AND to_time = '13:00' AND agent_id ='$agent' AND tech_dial_list_id ='$tech_dial_list_id'";
		...
		$stmt_count_13_15 = "SELECT * FROM dms_retailers_data WHERE Optional1 = '$date' AND  from_time = '13:00' AND to_time = '15:00' AND agent_id ='$agent' AND tech_dial_list_id ='$tech_dial_list_id'";
		...
		$stmt_count_15_18 = "SELECT * FROM dms_retailers_data WHERE Optional1 = '$date' AND  from_time = '15:00' AND to_time = '18:00' AND agent_id ='$agent' AND tech_dial_list_id ='$tech_dial_list_id'";
		...
		if(count($final_array)>0){
			$stmt_add_list="INSERT INTO vicidial_lists (list_id,list_name,campaign_id,active,list_description,list_changedate) values('$list_id','$list_name','$agent','$active','$list_description','$NOW_TIME');";
		...
		writeErroIntoLogFile('Selected agent End******** => '.$agent,$uploadDataLog);

		}

}else{
	    echo json_encode(array('success'=>true,'message'=>"No Agent Found"));
		writeErroIntoLogFile('No Agent Found => ',$uploadDataLog);
