...
if ($ACTION == 'LogiNCamPaigns' || $ACTION == 'LogiNCamPaigns_new')
	{
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ($show_campaign_list > 0)
		...
		while ($camps_to_print > $o) 
			{
			$rowx=mysqli_fetch_row($rslt);
			echo "<option  $selected value=\"$rowx[0]\"";
...
if ($ACTION == 'LogiNCamPaigns_new')
	...
	if ($show_campaign_list > 0)
        ...
        if($camps_to_print==1){
        	$rowx=mysqli_fetch_row($rslt);
        	 echo "<input type=hidden name=VD_campaign value=\"$rowx[0]\" >";
          
        }
		else{
	    echo "<td align=\"right\" valign=\"top\" style='width: 100%;float: left;text-align: left;'><font class=\"skb_text\">"._QXZ("Campaign:")."</font>  </td>";
        echo "<td align=\"left\" style='width: 100%;float: left;'><font class=\"skb_text\"><span id=\"LogiNCamPaigns2\">";		
        echo "<select size=1 name=VD_campaign id=VD_campaign style='width: 100%;'>\n";
		echo "<option value=\"\">-- "._QXZ("PLEASE SELECT A CAMPAIGN")." --</option>\n";
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			if ( ($CBstatus > 0) or (preg_match("/CALLBK|CBHOLD/i", $dispo)) )
				{
				$stmt="UPDATE vicidial_callbacks set status='INACTIVE' where lead_id='$lead_id' and list_id='$list_id' and status NOT IN('INACTIVE','DEAD','ARCHIVE');";
					if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_to_mysqli($stmt, $link);
